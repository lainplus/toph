...
impl Session {
    ...
    /// Name of toph initialization script.
    const INIT: &'static str = "init.toph";
    ...
    pub fn init(mut self, source: Option<PathBuf>) -> std::io::Result<Self> {
        ...
        self.message(format!("toph v{}", crate::VERSION), MessageType::Debug);
        ...
        for path in paths {
            ...
            if path.is_dir() {
                for entry in path.read_dir()? {
                    ...
                    if path.file_name() == Some(OsStr::new(".tophrc")) {
                        continue;
                    }
    ...
    /// Source an toph script at the given path. Returns an error if the path
